...
def extract_paragraphs(pdf_file) -> list[str]:
    """Extract non-empty paragraphs from a PDF, with OCR fallback for scanned PDFs."""
    paragraphs = []

    # ── Attempt 1: pdfplumber (fast, for text-based PDFs) ──────────────────────
    try:
    ...
    # ── Attempt 2: OCR via pytesseract (for scanned/image PDFs) ───────────────
    st.info("📷 No text layer found — switching to OCR mode (this may take a moment)…")
    try:
        import pytesseract
        ...
        full_text = ""
        for img in images:
            full_text += pytesseract.image_to_string(img, lang="eng+ron") + "\n\n"
        ...
        st.error(
            "OCR libraries not installed. Add these to your `requirements.txt`:\n"
            "```\npdf2image\npytesseract\n```\n"
            "And add a `packages.txt` file with:\n"
            "```\ntesseract-ocr\ntesseract-ocr-ron\npoppler-utils\n```"
        )
